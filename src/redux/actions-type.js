export const FILTER = 'FILTER'
export const GET_SHOE_BY_ID = 'GET_SHOE_BY_ID'
export const API_URL = 'https://ecommerce-zapatosapp.onrender.com'
export const FILTER_RANGE = 'FILTER_RANGE'
export const FILTER_LOCAL = 'FILTER_LOCAL'
export const CHANGE_PAGE = 'CHANGE_PAGE'
export const ORDER = 'ORDER'
export const POST_SHOE = 'POST_SHOE'

export const ADD_TO_SHOPPING_CART = 'ADD_TO_SHOPPING_CART'
export const SET_SHOPPING_CART = 'SET_SHOPPING_CART'

export const CREATE_PURCHASE_TICKET = 'CREATE_PURCHASE_TICKET';
export const UPDATE_PURCHASE_TICKET = 'UPDATE_PURCHASE_TICKET';

export const GET_USER_BY_EMAIL = 'GET_USER_BY_EMAIL';
export const CLEAN_USER_DATA = 'CLEAN_USER_DATA';

